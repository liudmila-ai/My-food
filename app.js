...
function renderAuth(mode='login',message=''){
  ...
  app.innerHTML=`<section class="auth-wrap"><div class="auth-card"><span class="eyebrow">Моя еда · v12</span><h2>${signup?'Создать аккаунт':'Войти'}</h2>${message?`<div class="auth-message">${esc(message)}</div>`:''}${signup?`<label class="field">Имя<input id="authName" autocomplete="name" placeholder="Как вас называть"></label>`:''}<label class="field">Email<input id="authEmail" type="email" autocomplete="email" placeholder="name@example.com"></label><label class="field">Пароль<input id="authPassword" type="password" autocomplete="${signup?'new-password':'current-password'}" placeholder="Минимум 6 символов"></label>${signup?`<label class="field">Повторите пароль<input id="authPassword2" type="password" autocomplete="new-password"></label>`:''}<button class="btn auth-primary" onclick="${signup?'signUp()':'signIn()'}">${signup?'Создать аккаунт':'Войти'}</button><button class="auth-link" onclick="renderAuth('${signup?'login':'signup'}')">${signup?'Уже есть аккаунт':'Создать аккаунт'}</button>${!signup?`<button class="auth-link" onclick="resetPassword()">Забыли пароль?</button>`:''}<p class="tiny-note">Первый вход и регистрация требуют доступа к Supabase. После входа приложение работает локально без сети/VPN и синхронизируется позже.</p></div></section>`;
}
...
function randomizeWeek(){const p=profile(),tar=target(),b=DISHES.filter(d=>d.category==='Завтрак'),sn=DISHES.filter(d=>d.category==='Перекус'),allM=DISHES.filter(d=>d.category==='Основное');const tpl=WEEK_TEMPLATES[Math.random()*WEEK_TEMPLATES.length|0];let mains=allM.filter(d=>tpl.proteins.includes(proteinFamily(d))&&tpl.carbs.includes(carbFamily(d)));if(mains.length<6)mains=allM.filter(d=>tpl.proteins.includes(proteinFamily(d)));
 ...
 DAYS.forEach((day,di)=>{let best,bestScore=1e9;for(let i=0;i<800;i++){const lunch=mains[Math.random()*mains.length|0],dinner=mains[Math.random()*mains.length|0],pp={breakfast:breakfastPool[di%breakfastPool.length].id,lunch:lunch.id,snack:snackPool[di%snackPool.length].id,dinner:dinner.id},t=totals(Object.values(pp));const repeat=(used[lunch.id]||0)*18+(used[dinner.id]||0)*18;const uniqueProtein=(proteinFamily(lunch)!==proteinFamily(dinner)?10:0);const st=SLOTS.reduce((a,[key])=>{const x=scaledDish(dish(pp[key]),key);a.protein+=x.protein;a.fat+=x.fat;a.carbs+=x.carbs;return a},{protein:0,fat:0,carbs:0});const score=Math.abs(st.protein-tar.protein)*5+Math.abs(st.fat-tar.fat)*4+Math.abs(st.carbs-tar.carbs)*2+repeat+(lunch.id===dinner.id?80:0)+uniqueProtein;if(score<bestScore){bestScore=score;best=pp}}p.plan[day]=best;[best.lunch,best.dinner].forEach(id=>used[id]=(used[id]||0)+1)});state.cookingDone={};save();render()}

function selectedPlans(){const plan=profile()?.plan||{};return participants().flatMap(p=>DAYS.flatMap(day=>SLOTS.map(([mealType])=>({profile:p,day,mealType,id:plan[day]?.[mealType]})).filter(x=>x.id)))}
...
function inferredPrepMeta(id){const d=dish(id),r=RECIPE_DETAILS[id]||{},explicit=(typeof PREP_META!=='undefined'&&PREP_META[id])||{};const txt=((r.storage||'')+' '+(d?.tags||[]).join(' ')).toLowerCase();let shelfDays=explicit.shelfDays||(/3\s*дн/.test(txt)?3:/2\s*дн/.test(txt)?2:3);const freezer=explicit.freezer!==undefined?explicit.freezer:/замораж|морозил/.test(txt);let mode=explicit.mode;if(!mode){if(r.prepType==='nocook')mode='assemble';else if(r.prepType==='daybefore')mode='daybefore';else if(r.prepType==='fresh')mode='fresh';else if(r.prepType==='batch')mode='batch_full';else mode='fresh'}return {...explicit,mode,shelfDays,freezer,prepMinutes:explicit.prepMinutes||Math.max(5,d?.minutes||15)}}
...
function cookingPlan(){const meals=mealAggregate();if(!meals.length)return[];const tasks=mergeMealTasks(meals),sessions=[...new Set(tasks.map(x=>x.session))].sort(sessionSort),groups=[];
 ...
 const bases=aggregatePrep().filter(x=>x.type==='protein'||x.type==='carb'||x.type==='veg');if(bases.length)groups.unshift({title:'Общие объёмы на неделю',time:'для закупки и партий',items:bases.map(x=>({key:'summary-'+x.type+'-'+x.label,label:`${x.label} · ${Math.round(x.qty)} г`,text:`Общий объём во всех порциях семьи. Используется в: ${[...x.examples].slice(0,5).join(', ')}.`}))});return groups}
